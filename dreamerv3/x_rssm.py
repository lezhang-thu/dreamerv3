...
class RSSM(nj.Module):
    ...
    def entry_space(self):
        return dict(deter=elements.Space(np.float32, self.deter),
                    stoch=elements.Space(np.float32,
                                         (self.stoch, self.classes)))

    def initial(self, bsize):
        carry = nn.cast(
            dict(deter=jnp.zeros([bsize, self.deter], f32),
    ...
    def observe(self, carry, tokens, action, reset, training, single=False):
        ...
        if single:
            carry, (entry, feat) = self._observe(carry, tokens, action, reset,
                                                 training)
    ...
    def _observe(self, carry, tokens, action, reset, training):
        ...
        for i in range(self.obslayers):
            ...
            x = nn.act(self.act)(self.sub(f'obs{i}norm', nn.Norm,
                                          self.norm)(x))
    ...
    def imagine(self, carry, policy, length, training, single=False):
        if single:
            ...
            assert all(x.dtype == nn.COMPUTE_DTYPE
                       for x in (deter, stoch, logit))
            return carry, (feat, action)
        else:
            unroll = length if self.unroll else 1
            if callable(policy):
                carry, (feat, action) = nj.scan(lambda c, _: self.imagine(
    ...
    def _core(self, deter, stoch, action):
        ...
        for i in range(self.dynlayers):
            x = self.sub(f'dynhid{i}', nn.BlockLinear, self.deter, g,
                         **self.kw)(x)
            x = nn.act(self.act)(self.sub(f'dynhid{i}norm', nn.Norm,
                                          self.norm)(x))
    ...
    def _prior(self, feat):
        ...
        for i in range(self.imglayers):
            ...
            x = nn.act(self.act)(self.sub(f'prior{i}norm', nn.Norm,
                                          self.norm)(x))
...
class Encoder(nj.Module):
    ...
    def __call__(self, carry, obs, reset, training, single=False):
        ...
        for i, depth in enumerate(self.depths):
            ...
            x = nn.act(self.act)(self.sub(f'cnn{i}norm', nn.Norm,
                                          self.norm)(x))
...
class Decoder(nj.Module):
    ...
    def __call__(self, carry, feat, reset, training, single=False):
        ...
        if self.bspace:
            ...
            x = nn.act(self.act)(self.sub('spnorm', nn.Norm,
                                          self.norm)(x0 + x1))
        for i, depth in reversed(list(enumerate(self.depths[:-1]))):
            x = x.repeat(2, -2).repeat(2, -3)
            x = self.sub(f'conv{i}', nn.Conv2D, depth, K, **self.kw)(x)

            x = nn.act(self.act)(self.sub(f'conv{i}norm', nn.Norm,
                                          self.norm)(x))
        x = x.repeat(2, -2).repeat(2, -3)
        kw = dict(**self.kw, outscale=self.outscale)
        ...
        split = np.cumsum([self.obs_space[k].shape[-1]
                           for k in self.imgkeys][:-1])
